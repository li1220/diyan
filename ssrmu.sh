...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/li1220/diyan/master/bbr.sh; then
...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="http://cq.diyan.xyz/erwm?text=${SSurl}"
...
ssr_link_qr(){
	...
	SSRQRcode="http://cq.diyan.xyz/erwm?text=${SSRurl}"
...
Set_config_user(){
	...
	read -e -p "(默认: diyan):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="diyan"
...
Set_config_port(){
	...
	read -e -p "(默认: 8080):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="8080"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: diyan.xyz):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="diyan.xyz"
...
Set_config_transfer(){
	...
	do
	echo
	echo -e "请输入要设置的用户 可使用的总流量上限(单位: GB, 1-999999 GB)"
	read -e -p "(默认: 无限):" ssr_transfer
	[[ -z "$ssr_transfer" ]] && ssr_transfer="999999" && echo && break
	echo $((${ssr_transfer}+0)) &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${ssr_transfer} -ge 1 ]] && [[ ${ssr_transfer} -le 999999 ]]; then
			echo && echo ${Separator_1} && echo -e "	用户总流量 : ${Green_font_prefix}${ssr_transfer} GB${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
			echo -e "${Error} 请输入正确的数字(1-999999)"
		fi
	else
		echo -e "${Error} 请输入正确的数字(1-999999)"
	fi
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/li1220/ToyoDAdoubiBackup/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/li1220/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/li1220/diyan/master/service/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/li1220/diyan/master/service/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/li1220/diyan/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/li1220/diyan/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/li1220/diyan/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/li1220/diyan/master/ssrmu.sh" && chmod +x ssrmu.sh
